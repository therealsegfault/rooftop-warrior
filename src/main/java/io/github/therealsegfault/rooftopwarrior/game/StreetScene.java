...
public class StreetScene {
    ...
    // Parallax multipliers applied to cameraX
    ...
    // Camera X — the world X that maps to screen X=0
    ...
    private void updateLadder(float delta) {
        float ladderTop = GROUND_Y + LADDER_H;
        if (!autoClimbing && Gdx.input.isKeyJustPressed(Keys.Q)) {
            autoClimbing = true;
        }
        if (autoClimbing) {
    ...
    private void updateStreet(float delta) {
        ...
        if (waveWorldX < WAVE_SCREEN_X) waveWorldX = WAVE_SCREEN_X; // left boundary
        ...
        // Ladder grab
        float ladderSX = LADDER_WORLD_X - cameraX();
        float waveSX   = WAVE_SCREEN_X;
        if (waveSX + WAVE_W > ladderSX && waveSX < ladderSX + LADDER_W && isGrounded) {
            onLadder     = true;
            autoClimbing = false;
            isGrounded   = false;
    ...
    public void draw() {
        ...
        // Sky BG — tiled, slow parallax
        batch.begin();
        float bgScale  = SH / BG_H;
        float bgDrawW  = BG_W * bgScale;
        float skyScroll = (cam * SKY_MULT) % bgDrawW;
        float bgX = -skyScroll;
    ...
    private void drawDetailRow(ShapeRenderer s, float scrollX) {
        ...
        while (x < SW + tileW) {
            ...
            // Wire connecting post tops with catenary sag
            ...
            for (int seg = 0; seg < segs; seg++) {
                ...
                float wx0 = x1 + (x2 - x1) * t0;
                float wx1 = x1 + (x2 - x1) * t1;
                float sag = 22f;
